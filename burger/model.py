...
class Burger1DModel(BaseModel):
    ...
    def step(self, grad_model=None):
        ...
        self._burger(grad_model)

    @BaseModel._training_loop
    def _burger(self, grad_model=None):
        ...
        curr_w = grad_model.field(samples)
    ...
    def write_output(self, output_folder):
        ...
        # draw_signal1D(samples, values, y_gt=ref, y_max=1.0)
        fig = draw_signal1D(samples, values, y_max=1.0)

        save_path = os.path.join(
            output_folder, f"t{self.timestep:03d}_values.png")
