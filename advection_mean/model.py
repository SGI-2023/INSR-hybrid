...
class Advection1DModel(BaseModel):
    ...
    def step(self):
        ...
        self.last_steps += [self.field_prev]
        self._advect()
    ...
    def _advect(self):
        ...
        grad_u0 = torch.zeros(grad_u.shape).to(grad_u.device)
        tot = sum(range(1, len(self.last_steps) + 1))
        for i, nn in enumerate(self.last_steps):
            grad_u0 += gradient(nn(samples), samples).detach() * (i + 1) / tot
